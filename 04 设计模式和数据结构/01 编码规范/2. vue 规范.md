...
Vue的编码规范分为了4个等级：必要，强烈推荐，推荐，谨慎使用.
为了保证工程的代码质量，我们要求全部遵守.
...
# 2. 组件的 data 必须是一个函数
在一个完整的vue工程中，只有根组件（app）是通过new Vue()方式挂载在DOM上.
根组件在项目中有且只有一个，只有它的data可以不是函数，就这一个特例. 
其他的所有组件都应该是一个完整的vue组件, 应该严格遵守组件规范.
反例
```js
...
# 3. 对于任何一个有输入的Vue组件来说，应该定义Prop，并且应该尽量详细， type，required，default
为了保证每一个组件的高度自包含和可复用行，组件的输入/输出必须明确.
反例
```js
// 这样做只有开发原型系统时可以接受
props: ['status']
...
# 4. 总是用 :key 配合 v-for
因为Vue会复用循环元素，不使用:key时，遇到重新排序的问题时可能会遇到预期之外的bug.
...
# 6. 避免 v-if 和 v-for 用在一起. 永远不要把 v-if 和 v-for 同时用在同一个元素上。
