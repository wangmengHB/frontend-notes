...
从计算机计算的角度直接使用 transform 的矩阵值，就可以直接描述变换了，但是这个过程对于用户来说非常难以解释。   
...
function qrDecompose (a) {
  ...
  let denom = Math.pow(a[0], 2) + pow(a[1], 2);               
  let scaleX = Math.sqrt(denom);                              // scaleX 一定是正值
  let scaleY = (a[0] * a[3] - a[2] * a[1]) / scaleX;          // scaleY 一定是正值
