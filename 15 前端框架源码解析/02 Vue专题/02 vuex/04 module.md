...
vuex和redux最大的不同是, vuex的store支持嵌套的树形结构, 而redux中的reducer只支持一层的结构.

使用单一的状态树来管理状态会导致状态树对象过于庞大，因此开发成规模的应用必须要讲状态树进行拆分. 在vuex中将这种拆分称为modules，每个module中都有自己的state, mutation, action, getter, 甚至是嵌套的子模块.
